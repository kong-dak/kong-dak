...
export async function getDailyQuestion() {
    return await local.get(`/api/daily-questions`);
}
...
/**데일리 질문 답변 생성 */
export async function createAnswers(questionId: number) {
...
export async function addEmoji(questionId: number, answerId: number) {
    return await local.patch(`/api/daily-questions/${questionId}/answers/${answerId}/emoji`);
}
...
export async function deleteReply(questionId: number, replyId: number) {
    return await local.post(`/api/daily-questions/${questionId}/replies/${replyId}`);
}
