...
import com.kongdak.domain.dailyquestion.dto.request.DailyAnswerRequest;
import com.kongdak.domain.dailyquestion.dto.request.DailyAnswerUpdateRequest;
import com.kongdak.domain.dailyquestion.dto.request.EmojiRequest;
import com.kongdak.domain.dailyquestion.dto.request.ReplyRequest;
import com.kongdak.domain.dailyquestion.dto.response.*;
import com.kongdak.domain.dailyquestion.service.DailyQuestionService;
...
public class DailyQuestionController {
    ...
    public BaseResponse<DailyQuestionWithAnswersResponse> getAnswers(
            @Parameter(description = "인증된 사용자 ID", hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails,
            @Parameter(description = "질문 ID", required = true) @PathVariable("questionId") Long questionId
    ) {
        return BaseResponse.ok(dailyQuestionService.getDailyQuestionDetail(
                userDetails.getId(),
                questionId
        ));
    }
    ...
    public BaseResponse<DailyAnswerCreateResponse> createAnswer(
            @Parameter(description = "인증된 사용자 ID", hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails,
            @Parameter(description = "질문 ID", required = true) @PathVariable("questionId") Long questionId,
            @Parameter(description = "답변 내용") @RequestBody @Valid DailyAnswerRequest request
    ) {
        return BaseResponse.created(dailyQuestionService.createAnswer(
                userDetails.getId(),
                questionId,
                request
        ));
    ...
    public BaseResponse<DailyAnswerUpdateResponse> updateAnswer(
            @Parameter(description = "인증된 사용자 ID", hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails,
            @Parameter(description = "질문 ID", required = true) @PathVariable("questionId") Long questionId,
            @Parameter(description = "답변 ID", required = true) @PathVariable("answerId") Long answerId,
            @Parameter(description = "답변 내용") @RequestBody @Valid DailyAnswerUpdateRequest request
    ) {
        return BaseResponse.ok(dailyQuestionService.updateAnswer(
                userDetails.getId(),
                questionId,
                answerId,
                request
    ...
    @ApiResponse(responseCode = "200", description = "이모지 추가 성공", content = @Content(schema = @Schema(implementation = BaseResponse.class)))
    public BaseResponse<AnswerEmojiResponse> addEmoji(
            @Parameter(description = "인증된 사용자 ID", hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails,
            @Parameter(description = "질문 ID", required = true) @PathVariable("questionId") Long questionId,
            @Parameter(description = "답변 ID", required = true) @PathVariable("answerId") Long answerId,
            @Parameter(description = "이모지 정보") @RequestBody @Valid EmojiRequest request
    ) {

        return BaseResponse.ok(dailyQuestionService.addEmoji(
                userDetails.getId(),
                answerId,
                request
        ));
    ...
    @ApiResponse(responseCode = "200", description = "댓글 작성 성공", content = @Content(schema = @Schema(implementation = BaseResponse.class)))
    @PostMapping("/{questionId}/replies")
    public BaseResponse<ReplyCreateResponse> addReply(
            @Parameter(description = "인증된 사용자 ID", hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails,
            @Parameter(description = "질문 ID", required = true) @PathVariable("questionId") Long questionId,
            @Parameter(description = "댓글 내용") @RequestBody @Valid ReplyRequest request
    ) {

        return BaseResponse.created(dailyQuestionService.addReply(
                userDetails.getId(),
                questionId,
                request
        ));
    ...
    public BaseResponse<ReplyDeleteResponse> deleteReply(
            @Parameter(description = "인증된 사용자 ID", hidden = true) @AuthenticationPrincipal CustomUserDetails userDetails,
            @Parameter(description = "질문 ID", required = true) @PathVariable("questionId") Long questionId,
            @Parameter(description = "댓글 ID", required = true) @PathVariable("replyId") Long replyId
    ) {

        return BaseResponse.ok(dailyQuestionService.deleteReply(
                userDetails.getId(),
                questionId,
                replyId
        ));
